...
function createEmbed(options: ViewerOptions) {
  ...
  options.data[options.selected].forEach((player: any) => {
  ...
  if (options.data[options.selected][0] instanceof Skill) {
    ...
    options.data[options.selected][0] instanceof Activity ||
...
function createAllButtons(stats: string[]) {
...
function addNextPrevButtons(buttons: MessageButton[], page: number) {
  const nextButton = new MessageButton();
  nextButton.customId = "next";
  nextButton.label = "NEXT";
  nextButton.style = "SUCCESS";
  if (page === 3) nextButton.disabled = true;
...
function chunk<T>(arr: T[], limit: number): T[][] {
  const numChunks = Math.ceil(arr.length / limit);
  return Array.from({ length: numChunks }, (_, i) =>
    arr.slice((i * arr.length) / numChunks, ((i + 1) * arr.length) / numChunks)
  );
}

function createMessageComponents(stats: string[], page: number) {
  const allButtons = createAllButtons(stats);
  const pageButtons = chunk(
    addNextPrevButtons(allButtons.splice(10 * page, 10), page),
    5
  );
  return pageButtons.map((row) => new MessageActionRow().addComponents(row));
}

export function createUpdatedMessage(viewerOptions: ViewerOptions) {
